...
- **Proposed fix:** Two layers. (1) Per-project `.claude/settings.local.json` with broad allow for routine local operations (Bash patterns: `git*` minus push, `python3*`, `node*`, `npm*`, `mv*`, `mkdir*`, `cp*`, `ln*`, `rm*`, `ls*`, `cat*`, `grep*`, etc.), explicit `deny` for `git push*`, `git push --force*`, `gh pr create*`, `gh release*`, `rm -rf /*`, `rm -rf ~*`. Both budai and CanvasOS now ship this template at `.claude/settings.local.json` (gitignored — it's a local override). (2) Within budai's own workflow, finding F025 (auto-flip frontmatter on role completion) closes the other half of the approval volume — gate flips become runner-internal, not human-typed.
